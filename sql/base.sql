DROP DATABASE app_db;
CREATE DATABASE app_db;
USE app_db;

CREATE TABLE `user` (
                        `id` integer PRIMARY KEY AUTO_INCREMENT,
                        `username` varchar(150) UNIQUE NOT NULL,
                        `firstname` varchar(150),
                        `email` varchar(300) UNIQUE,
                        `surname` varchar(150),
                        `password` char(128) NOT NULL,
                        `date_joined` datetime,
                        `last_login` datetime,
                        `is_active` boolean,
                        `profile_picture` varchar(300)
);

CREATE TABLE `chat_room` (
                             `id` integer PRIMARY KEY AUTO_INCREMENT,
                             `name` varchar(150) NOT NULL
);

CREATE TABLE `chat_room_user` (
                                  `id` integer PRIMARY KEY AUTO_INCREMENT,
                                  `chat_room_id` integer NOT NULL,
                                  `user_id` integer NOT NULL
);

CREATE TABLE `message` (
                           `id` integer PRIMARY KEY AUTO_INCREMENT,
                           `user_id` integer NOT NULL,
                           `chat_room_id` integer NOT NULL,
                           `content` varchar(500) NOT NULL,
                           `sent_date` datetime
);

ALTER TABLE `chat_room_user` ADD FOREIGN KEY (`chat_room_id`) REFERENCES `chat_room` (`id`);

ALTER TABLE `chat_room_user` ADD FOREIGN KEY (`user_id`) REFERENCES `user` (`id`);

ALTER TABLE `message` ADD FOREIGN KEY (`user_id`) REFERENCES `user` (`id`);

ALTER TABLE `message` ADD FOREIGN KEY (`chat_room_id`) REFERENCES `chat_room` (`id`);

# create debug users
INSERT INTO user ( username, password)
VALUES ('user1','619f84fdd982e31571c30a2ed9d7b402');

INSERT INTO user ( username, password)
VALUES ('user2','619f84fdd982e31571c30a2ed9d7b402');

INSERT INTO user ( username, password)
VALUES ('user3','619f84fdd982e31571c30a2ed9d7b402');

INSERT INTO user ( username, password)
VALUES ('user4','619f84fdd982e31571c30a2ed9d7b402');


# create debug chat rooms
INSERT INTO chat_room ( name)
VALUES ('chat_room1');

INSERT INTO chat_room ( name)
VALUES ('chat_room2');

INSERT INTO chat_room ( name)
VALUES ('chat_room3');

# create debug chatroom relations

# chat_room1 has user1, user2, user3
INSERT INTO chat_room_user ( chat_room_id, user_id)
VALUES (1,1);

INSERT INTO chat_room_user ( chat_room_id, user_id)
VALUES (1,2);

INSERT INTO chat_room_user ( chat_room_id, user_id)
VALUES (1,3);

# chat_room2 has user1, user2
INSERT INTO chat_room_user ( chat_room_id, user_id)
VALUES (2,1);

INSERT INTO chat_room_user ( chat_room_id, user_id)
VALUES (2,2);


# chat_room3 has user3, user2, user4
INSERT INTO chat_room_user ( chat_room_id, user_id)
VALUES (3,3);

INSERT INTO chat_room_user ( chat_room_id, user_id)
VALUES (3,2);

INSERT INTO chat_room_user ( chat_room_id, user_id)
VALUES (3,4);

# debug messages
INSERT INTO  message ( user_id, chat_room_id, content, sent_date)
VALUES (1,1,'Hello world!', '2016-01-01 00:00:00');

INSERT INTO  message ( user_id, chat_room_id, content, sent_date)
VALUES (2,1,'HELLO BACK', '2016-01-01 00:00:01');

INSERT INTO  message ( user_id, chat_room_id, content, sent_date)
VALUES (4,2,'HELLO 1!', '2016-01-01 00:00:01');

INSERT INTO  message ( user_id, chat_room_id, content, sent_date)
VALUES (3,2,'HELLO 2!', '2016-01-01 00:00:01');